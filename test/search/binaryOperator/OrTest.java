...
class OrTest {
	...
	void setUp() {
		...
		or = new Or(leftClause, rightClause);
	}

	@Test
	void testAOrIsCreated() {
		assertEquals(leftClause, or.getLeftClause());
		assertEquals(rightClause, or.getRightClause());

	}

	@Test
	void testOr() {
		assertEquals(new HashSet<>(List.of(tripOne, tripTwo)),
				new HashSet<>(or.filterTrips(List.of(tripOne, tripTwo))));
